...
function closeLeague(leagueId) {
    fetch(`/close_league/${leagueId}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' }
    })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                ...
                alert(data.error || '마감에 실패했습니다.');
            }
        })
        .catch(error => console.error('Error closing league:', error));
}
